...
class Home < Wt::WApplication
  ...
  def initialize(env)
    ...
    for i in 0...@@languages.size
      ...
      a = Wt::WAnchor.new("", l.longDescription, languagesDiv)
      a.refInternalPath = l.path
    end
    ...
    @mainMenu.addItem(tr("introduction"), introduction).pathComponent = ""
  ...
  def readNews(newsTable, newsfile)
    ...
    while line = f.gets do
      ...
      newsTable.elementAt(row, 0).resize(Wt::WLength.new(16, Wt::WLength::FontEx), Wt::WLength::Auto)
...
Wt::WRun(ARGV) do |env|
  ...
  historyKey = env.getParameter("historyKey")
