...
'QGraphicsLayoutItem' => 'QGraphicsLayoutItemInterface',
...
sub resolveType {
	...
	return $prefix.$r.$suffix if ($r eq $argType); # oops, some strange recursion here
	...
	return $prefix.$r.$suffix if ($node->{Access} eq 'private' || $node->{Access} eq 'protected' || $argType eq 'enum');
